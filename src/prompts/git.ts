      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "",
              "Read the PR metadata with get_pull_request (title, source/target branches, reviewers, status) and its changed-file list with get_commit_changes. Then write a concise summary covering: its purpose, the key files it touches, and its overall scope.",
              "",
      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "",
              "This task is ADVISORY and READ-ONLY. Do NOT post comments, vote, approve, or call any write tool — produce the review as your reply only. Use only this server's read tools; do not fetch raw URLs or call the Azure DevOps REST API directly (a raw call won't carry this session's auth and will just hit a login page).",
              "",
              "Gather context: get_pull_request for metadata, then get_commit_changes for the changed-file list. Read the contents of only the MOST RELEVANT changed files with get_file_content — not every file — to keep token cost down; large files may be truncated by the tool, which is fine.",
              "",
